...
function PrintJSON({
  ...
  jsonData: { [id: string]: string | boolean | {} };
}) {
...
function PrintIcalData({ icalData }: { icalData: icalDataProps }) {
  ...
  taskIds.forEach((id) => {
    const isUserEvent =
      icalData[id].CATEGORIES !== undefined &&
      icalData[id].CATEGORIES == "ユーザー登録イベント";
    const isValidSummary =
      icalData[id].SUMMARY !== undefined &&
      icalData[id].SUMMARY.match(new RegExp(env.regExpValidEvent)) !== null;
    const isValidLimit =
      icalData[id].DTEND !== undefined &&
      (new Date(icalData[id].DTEND).getTime() - today.getTime()) / 86400000 >
        -3;
    ...
    reactNodeList.push(
      <Box
        ...
        css={css`
          margin: 5px 0;
          background-color: ${display ? "#e0fce7" : "#ff57572e"};
        `}
      >
        <p>
          <b>ID</b>: {id}
        ...
        <p>
          <b>CATEGORIES:</b> {icalData[id]?.CATEGORIES}
        </p>
        <p>
          <b>SUMMARY</b>: {icalData[id]?.SUMMARY}
        </p>
        <p>
          <b>DESCRIPTION</b>: {icalData[id]?.DESCRIPTION}
        </p>
        <p>
          <b>CLASS</b>: {icalData[id]?.CLASS}
        </p>
        <p>
          <b>DTEND</b>: {dayjs(icalData[id]?.DTEND).format("YYYY/MM/DD HH:mm")}
        </p>
        <p>
          <b>有効イベント判定</b>: {eventType}
        </p>
        <p>
          <b>有効期限判定</b>: {isValidLimit ? "表示期限内" : "期限切れ"}
        </p>
      </Box>,
    );
  });
  ...
  if (reactNodeList.length == 0) {
    reactNodeList.push(<p key="0">データなし</p>);
  }
  ...
  taskIds.forEach((id) => {
    reactNodeList.push(
      <Box
        ...
        css={css`
          margin: 5px 0;
          background-color: ${saveData[id].display ? "#e0fce7" : "#ff57572e"};
        `}
      >
        <p>
          <b>ID</b>: {id}
        ...
        <p>
          <b>className:</b> {saveData[id].className}
        </p>
        <p>
          <b>taskName</b>: {saveData[id].taskName}
        </p>
        <p>
          <b>taskLimit</b>: {saveData[id].taskLimit?.fullDate}{" "}
          {saveData[id].taskLimit?.time}
        </p>
        <p>
          <b>finish</b>: {String(saveData[id].finish)}
        </p>
      </Box>,
    );
  });
...
function Debugger() {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      setDebugData({
        ...
        icalData_g: icalData_g.status == "success" ? icalData_g.data : {},
        moodleURL_s: moodleURL.s,
        moodleURLConnection_s: moodleURLConnection_s,
        icalData_s: icalData_s.status == "success" ? icalData_s.data : {},
  ...
  return (
    <Box padding="10px 20px" fontSize="16px">
      ...
      <p>
        本ツールが正しく課題データを取得できているかを確認するためのページです。
      </p>
      <p>
        バグの報告等は、このページのスクリーンショットを撮影して共有してください。
      </p>
      <p
        css={css`
          font-weight: bold;
      ...
      >
        ※
        課題データ（URL1）と課題データ（URL2）で、課題提出期限が設定されているイベントであるのに無効イベントとして表示される場合は、以下までお知らせください。
      </p>
      <p
        css={css`
          font-weight: bold;
        ...
        `}
      >
        お問い合わせ：racsu.shinshu.univ[at]gmail.com
      </p>
      ...
      <Section>
        ...
        <p>
          {debugData.moodleURLConnection_g
            ? "正常接続"
            : "接続できませんでした"}
        </p>
      </Section>
      ...
      <Section>
        ...
        <p>
          {debugData.moodleURLConnection_s
            ? "正常接続"
            : "接続できませんでした"}
        </p>
      </Section>
...
ReactDOM.createRoot(document.getElementById("root")!).render(<Debugger />);
