...
const icalParser = (icalText: string) => {
  ...
  icalText
    .replace(/\\n|\n|\t|\r| |　|\\|END:VEVENT|END:VCALENDAR/g, "")
    .split("BEGIN:VEVENT")
    .forEach((eachEvent) => {
      const eventData: { [index: string]: string } = {};
      let uid = "";
      eachEvent
        .split(new RegExp(env.regExpIcalSplitPattern, "g"))
        .forEach((eachEventDetail) => {
          const eventDetail = eachEventDetail.match(
            new RegExp(env.regExpIcalIndexPattern),
          );
          if (eventDetail !== null && eventDetail.groups !== undefined) {
            if (eventDetail.groups.index == "UID") {
              ...
              eventData[eventDetail.groups.index] = formatDate(
                eventDetail.groups.data,
              );
            } else {
...
export const getAccountParams = (url: string) => {
  ...
  if (urlParams == null || urlParams.groups == undefined) {
    ...
    return {
      expiration: urlParams.groups.expiration,
      department: urlParams.groups.department,
      userid: urlParams.groups.userid,
      authtoken: urlParams.groups.authtoken,
    };
  }
};
...
export const getMoodleURL = ({
  ...
  return env.moodleURL
    .replace("$accountExpiration", accountExpiration)
    .replace("$department", userDepartment)
    .replace("$moodleId", moodleId)
    .replace("$moodleToken", moodleToken);
};
