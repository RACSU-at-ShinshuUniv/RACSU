...
export class SyllabusClient {
  ...
  getClassName = async (classCode: string) => {
    if (classCode in this.classNameDict) {
      console.log(
        `ローカルデータから取得：${classCode}->${this.classNameDict[classCode]}`,
      );
      return this.classNameDict[classCode];
    } else {
      const className = `不明な授業（${classCode}）`;
      this.classNameDict[classCode] = className;
      return className;

      // 2026/06/28 キャンパス情報システムの仕様変更で、以下の授業名取得処理が使えなくなったため、修正までコメントアウト
      ...
      // const syllabusUrl = env.syllabusURL
      //   .replace("$term", String(term.getFullYear()))
      //   .replace("$department", department)
      //   .replace("$classCode", classCodeFixed);

      // const syllabusContents = await (await fetch(syllabusUrl)).text();
      // const detectedClassName_A = syllabusContents
      //   .replace(/\n|\r\n|\t| /g, "")
      //   .match(/授業名<\/td><tdcolspan="7">(?<name>.*?)<\/td>/);
      // const detectedClassName_B = syllabusContents
      //   .replace(/\n|\r\n|\t| /g, "")
      //   .match(/科目名<\/td><tdcolspan="1">(?<name>.*?)<\/td>/);

      // const className = (() => {
      //   if (
      //     detectedClassName_A !== null &&
      //     detectedClassName_A.groups !== undefined &&
      //     detectedClassName_A.groups.name !== undefined
      //   ) {
      //     return detectedClassName_A.groups.name;
      //   } else if (
      //     detectedClassName_B !== null &&
      //     detectedClassName_B.groups !== undefined &&
      //     detectedClassName_B.groups.name !== undefined
      //   ) {
      //     return detectedClassName_B.groups.name;
      //   } else {
      //     return `不明な授業（${classCode}）`;
      //   }
      // })();
  ...
  overwriteIcalClassCode = async (icalData: {
    [uid: string]: { [index: string]: string };
  }) => {
    const overwriteIcalData = await Promise.all(
      Object.keys(icalData).map((key) => {
